...
export function SpinWidget({
  ...
  async function onSpin() {
    if (busy || celebrating || prizes.length === 0 || !claimReady) return;
    ...
    // Fixed split so wall-clock animation == admin setting (API already resolved).
    ...
    // Animation only — prize is already known, so duration matches 3s/5s/7s exactly.
    setMotion("spinning");
    ensureWinAudio();
    ensureConfettiAudio();
    startSpinSound();
